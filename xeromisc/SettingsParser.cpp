...
namespace xero
{
	namespace misc 
	{
		...
		bool SettingsParser::getBoolean(const std::string &key) const {
			return getSetting(key).getBoolean() ;
		}

		int SettingsParser::getInteger(const std::string &key) const {
			return getSetting(key).getInteger();
		}

		double SettingsParser::getDouble(const std::string &key) const {
			return getSetting(key).getDouble();
		}

		const std::string &SettingsParser::getString(const std::string &key) const {
			return getSetting(key).getString();
		}

		const Setting &SettingsParser::getSetting(const std::string &key) const {
			auto it = settings_.find(key) ;
			if (it == settings_.end()) {
				logger_.startMessage(MessageLogger::MessageType::debug) ;
				logger_ << "SettingsParser: variable '" << key << "' was not found - aborting" ;
				logger_.endMessage() ;				
			}
			return it->second ;
		}
