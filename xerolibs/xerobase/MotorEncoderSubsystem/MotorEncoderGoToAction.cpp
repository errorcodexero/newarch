...
namespace xero {
    namespace base {
        MotorEncoderGoToAction::MotorEncoderGoToAction(MotorEncoderSubsystem &subsystem, double target):
            ...
            std::string config = subsystem.config_name_
        ...
        void MotorEncoderGoToAction::start() {
            ...
            std::string config = subsystem.config_name_ + ":follower";
            auto &settings = subsystem.getRobot().getSettingsParser();
        ...
        void MotorEncoderGoToAction::run() {
            ...
            if (isDone_) {
                double dist = target_ - subsystem.getHeight();
                double out = ctrl_.getOutput(0, dist, subsystem.getRobot().getDeltaTime());
