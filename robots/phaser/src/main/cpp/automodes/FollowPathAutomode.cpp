...
namespace xero {
    namespace phaser {
        FollowPathAutomode::FollowPathAutomode(Robot &robot, const std::string &name) : PhaserAutoModeBase(robot, "Followpath  - " + name, "Follow a path")
        {            
            auto &phaser = dynamic_cast<Phaser &>(getRobot()) ;
            auto phaserrobot = phaser.getPhaserRobotSubsystem() ;
            auto db = phaserrobot->getTankDrive() ;

            pushSubActionPair(db, std::make_shared<TankDriveFollowPathAction>(*db, name)) ;
        }
